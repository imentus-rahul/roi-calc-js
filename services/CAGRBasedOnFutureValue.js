...
function calculateCompoundInterestRate(principal, actualFutureValue, nper, annualPayment, number_of_years_recursive_payment_made) {
    let lowerRateBound = new Decimal(0);
    let upperRateBound = new Decimal(1);

    for (let i = 0; i < 100; i++) { // Limit iterations to avoid infinite loop
        const rateGuess = (lowerRateBound.plus(upperRateBound)).div(2);
        console.log("🚀 ~ file: index.js:34 ~ calculateCompoundInterestRate ~ rateGuess:", rateGuess);

        let calculatedFutureValue = new Decimal(principal);
        for (let year = 1; year <= nper; year++) {
            let recursive_annual_principle_paid = new Decimal(annualPayment); // cloning in other variable, so that it can be reused in nested for loops
            if (year > number_of_years_recursive_payment_made) {
                recursive_annual_principle_paid = new Decimal(0);
            }
            calculatedFutureValue = calculatedFutureValue.plus(recursive_annual_principle_paid);
            calculatedFutureValue = calculatedFutureValue.plus(calculatedFutureValue.mul(rateGuess));
        }

        console.log(`🚀 ~ file: index.js:45 ~ calculateCompoundInterestRate ~ calculatedFutureValue: ${calculatedFutureValue.toFixed(9)}, at iteration i: ${i} `);
        ...
        if (calculatedFutureValue.minus(actualFutureValue).abs().toFixed(9) == 0 || calculatedFutureValue.minus(actualFutureValue).abs().toFixed(9) <= 0.000_000_011) { // TODO: check logic here, so that sum should always be greater then or equal to fv and less then (fv+0.000_000_010) difference
            return rateGuess.mul(100); // Convert rate to percentage
        }
...
export function inputsForCalculateCompoundInterestRate() {
    // Input values
...
export default { inputsForCalculateCompoundInterestRate };
